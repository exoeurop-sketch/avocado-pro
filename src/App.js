...
function Tag({label,active,onClick,color="#4ade80"}){return <button onClick={onClick} style={{padding:"5px 11px",borderRadius:20,border:`1px solid ${active?color:"#1e2d3d"}`,background:active?`${color}18`:"transparent",color:active?color:"#6b7280",fontSize:12,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>{label}</button>;}
...
function LoginPage({t}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#07101a",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'DM Sans',sans-serif"}}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;700;800&display=swap" rel="stylesheet"/>
      <div style={{marginBottom:24,textAlign:"center"}}>
        <div style={{fontSize:44,marginBottom:12}}>🥑</div>
        <h1 style={{fontSize:24,fontWeight:800,margin:0}}>Hass<span style={{color:"#4ade80"}}>Market</span> Pro</h1>
        <p style={{fontSize:13,color:"#6b7280",marginTop:6}}>{t.loginSub}</p>
      </div>
      <div style={{background:"#0b1624",borderRadius:18,border:"1px solid #1a2740",padding:"32px 28px",width:"100%",maxWidth:380}}>
        <h2 style={{fontSize:18,fontWeight:700,marginBottom:20}}>{t.loginTitle}</h2>
        <form onSubmit={handleLogin}>
          <div style={{marginBottom:12}}>
            <label style={{fontSize:11,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.06em",display:"block",marginBottom:5}}>{t.loginEmail}</label>
            <input type="email" value={email} onChange={e=>setEmail(e.target.value)} required style={{width:"100%",padding:"11px 14px",borderRadius:10,border:"1px solid #1a2740",background:"#07101a",color:"white",fontSize:14,fontFamily:"inherit",outline:"none"}}/>
          </div>
          <div style={{marginBottom:16}}>
            <label style={{fontSize:11,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.06em",display:"block",marginBottom:5}}>{t.loginPass}</label>
            <input type="password" value={pass} onChange={e=>setPass(e.target.value)} required style={{width:"100%",padding:"11px 14px",borderRadius:10,border:"1px solid #1a2740",background:"#07101a",color:"white",fontSize:14,fontFamily:"inherit",outline:"none"}}/>
          </div>
          {error&&<div style={{fontSize:12,color:"#f87171",marginBottom:12,background:"#1a0a0a",borderRadius:8,padding:"8px 12px"}}>{error}</div>}
          <button type="submit" disabled={loading} style={{width:"100%",padding:"13px",borderRadius:10,border:"none",background:loading?"#1a2740":"linear-gradient(135deg,#166534,#15803d)",color:loading?"#4b5563":"white",fontSize:14,fontWeight:700,cursor:loading?"not-allowed":"pointer",fontFamily:"inherit"}}>
            {loading?"⏳...":t.loginBtn}
          </button>
        </form>
      </div>
      <div style={{marginTop:16,fontSize:11,color:"#374151"}}>HassMarket Pro · Accès sur invitation uniquement</div>
    </div>
  );
}
...
function Dashboard({userEmail,isAdmin,lang,setLang}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#07101a",color:"#f1f5f9",fontFamily:"'DM Sans',sans-serif",paddingBottom:60}}>
      ...
      <div style={{background:"linear-gradient(135deg,#04200f,#0b1a10,#07101a)",padding:"14px 16px",borderBottom:"1px solid #0f2918"}}>
        <div style={{maxWidth:1000,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8,marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <span style={{fontSize:22}}>🥑</span>
              <span style={{fontSize:18,fontWeight:800,letterSpacing:"-0.02em"}}>Hass<span style={{color:"#4ade80"}}>Market</span> Pro</span>
            ...
            <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
              <div style={{display:"flex",gap:3,background:"#0b1624",border:"1px solid #1a2740",borderRadius:8,padding:"3px 4px"}}>
              ...
              <div style={{background:"#0b1624",border:"1px solid #1a2740",borderRadius:8,padding:"5px 10px",fontSize:10}}>
                ...
                {ratesDate&&<div style={{color:"#374151",fontSize:9}}>{t.updated}: {ratesDate}{rates.USD?` · 1€=${rates.USD.toFixed(3)}$`:""}</div>}
              </div>
              <div style={{display:"flex",alignItems:"center",gap:6,background:"#0b1624",border:"1px solid #1a2740",borderRadius:8,padding:"5px 10px"}}>
                <span style={{fontSize:10,color:"#6b7280",maxWidth:130,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{userEmail}</span>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {t.kpi.map((k,i)=>(
              <div key={i} style={{flex:1,minWidth:120,background:"linear-gradient(145deg,#0b1624,#101f30)",border:`1px solid ${["#f59e0b","#f87171","#4ade80","#60a5fa"][i]}20`,borderRadius:10,padding:"10px 12px",position:"relative",overflow:"hidden"}}>
                <div style={{position:"absolute",bottom:-4,right:-4,fontSize:30,opacity:0.07}}>{["📦","📊","🚢","💰"][i]}</div>
                <div style={{fontSize:9,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:2}}>{k.label}</div>
      ...
      <div style={{maxWidth:1000,margin:"0 auto",padding:"12px 14px 0"}}>
        <div style={{display:"flex",gap:6,marginBottom:10,overflowX:"auto",paddingBottom:4}}>
          {t.tabs.map((lbl,i)=><button key={i} onClick={()=>setTab(i)} style={{padding:"8px 14px",borderRadius:10,border:"1px solid",borderColor:tab===i?"#4ade80":"#1a2740",background:tab===i?"#4ade8018":"transparent",color:tab===i?"#4ade80":"#6b7280",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>{lbl}</button>)}
        </div>
        ...
        {tab===0&&(<div>
          <div style={{display:"flex",gap:10,marginBottom:10,flexWrap:"wrap"}}>
            ...
            <div style={{background:"#0b1624",borderRadius:12,border:"1px solid #1a2740",padding:"12px 14px"}}>
              ...
              <div style={{display:"flex",gap:5}}>{allWeeks.map(w=><button key={w} onClick={()=>setWeek(w)} style={{padding:"5px 12px",borderRadius:8,border:`1px solid ${week===w?"#4ade80":"#1a2740"}`,background:week===w?"#4ade8018":"transparent",color:week===w?"#4ade80":"#6b7280",fontSize:13,cursor:"pointer",fontFamily:"'Space Mono',monospace",fontWeight:700}}>S{w}</button>)}</div>
            </div>
          </div>
          <div style={{background:"#0b1624",borderRadius:14,border:"1px solid #1a2740",overflow:"hidden"}}>
            ...
            {CALIBRES_LIST.map((c,i)=>{const p=REAL_PRICES[week]?.[origin]?.[c];const tr=getTrend(origin,c);const isSel=cal===c;const isKg=c>=26;return(
              <div key={c} onClick={()=>setCal(c)} style={{display:"grid",gridTemplateColumns:"60px 90px 110px 110px 90px",padding:"10px 14px",cursor:"pointer",background:isSel?`${cfg.color}12`:i%2===0?"#0b1624":"#0d1a28",borderLeft:isSel?`3px solid ${cfg.color}`:"3px solid transparent",borderBottom:i<CALIBRES_LIST.length-1?"1px solid #1a2740":"none"}}>
                ...
                <div style={{fontFamily:"'Space Mono',monospace",fontSize:13,color:"#4ade80",fontWeight:700,display:"flex",alignItems:"center"}}>{p?`${p[0]}${isKg?" €/kg":"€"}`:<span style={{color:"#374151"}}>{t.nd}</span>}</div>
                <div style={{fontFamily:"'Space Mono',monospace",fontSize:13,color:"#34d399",fontWeight:700,display:"flex",alignItems:"center"}}>{p?`${p[1]}${isKg?" €/kg":"€"}`:""}</div>
                <div style={{display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:16,color:trendColor(tr),fontWeight:700}}>{tr}</span><span style={{fontSize:9,color:"#374151"}}>S{allWeeks[0]}→S{allWeeks[allWeeks.length-1]}</span></div>
              </div>);})}
          </div>
          {ctx&&(<div style={{background:"#0b1624",borderRadius:12,border:"1px solid #1a2740",padding:14,marginTop:10}}>
        ...
        {tab===1&&(<div>
          <div style={{background:"#0b1624",borderRadius:14,border:"1px solid #1a2740",padding:16,marginBottom:10}}>
            ...
            <div style={{display:"flex",alignItems:"flex-end",gap:5,height:90}}>
              {Object.entries(WEEKLY_SUPPLY_EU).map(([w,v])=>{const isReal=REAL_WEEKS.includes(parseInt(w));const isPeak=parseInt(w)>=18;return(<div key={w} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                ...
                <div style={{width:"100%",borderRadius:"3px 3px 0 0",background:isReal?(isPeak?"#f59e0b":"#4ade80"):"#1a2740",height:`${(v/maxSupply)*74}px`,minHeight:5,opacity:isReal?1:0.5,border:!isReal?"1px dashed #374151":"none"}}/>
          ...
          <div style={{background:"#0b1624",borderRadius:14,border:"1px solid #1a2740",padding:16,marginBottom:10}}>
            ...
            <div style={{marginTop:10,background:"#07101a",borderRadius:8,padding:"8px 12px",fontSize:11,color:"#f87171"}}>⚠️ {t.ciradAlert}</div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {t.blocks.map(b=><div key={b.title} style={{background:"#0b1624",borderRadius:12,border:`1px solid ${b.color}20`,padding:14}}>
              <div style={{fontSize:12,fontWeight:700,color:b.color,marginBottom:10}}>{b.title}</div>
              {b.items.map(item=><div key={item} style={{fontSize:11,color:"#9ca3af",marginBottom:5,paddingLeft:10,borderLeft:`2px solid ${b.color}40`}}>{item}</div>)}
            </div>)}
          </div>
        </div>)}

        {tab===2&&(<div style={{background:"#0b1624",borderRadius:14,border:"1px solid #1a2740",overflow:"hidden"}}>
          <div style={{padding:"10px 14px",borderBottom:"1px solid #1a2740",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:6}}>
          ...
          {TOP_IMPORTERS.map((imp,i)=>{const growth=((imp.w18-imp.w17)/imp.w17*100);const barW=(imp.w18/TOP_IMPORTERS[0].w18)*100;return(
            <div key={imp.name} style={{display:"grid",gridTemplateColumns:"28px 1fr 36px 80px 80px 72px",padding:"10px 14px",position:"relative",background:i%2===0?"#0b1624":"#0d1a28",borderBottom:i<TOP_IMPORTERS.length-1?"1px solid #1a2740":"none"}}>
        ...
        {tab===3&&(<div>
          <div style={{display:"flex",gap:10,marginBottom:12,flexWrap:"wrap"}}>
            ...
            <div style={{background:"#0b1624",borderRadius:12,border:"1px solid #1a2740",padding:"12px 14px"}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:4}}>{CALIBRES_LIST.map(c=><button key={c} onClick={()=>setCal(c)} style={{padding:"3px 8px",borderRadius:6,border:`1px solid ${cal===c?"#4ade80":"#1a2740"}`,background:cal===c?"#4ade8018":"transparent",color:cal===c?"#4ade80":"#6b7280",fontSize:12,cursor:"pointer",fontFamily:"'Space Mono',monospace",fontWeight:700}}>{c}</button>)}</div>
            </div>
          </div>
          <div style={{background:"linear-gradient(135deg,#0b1624,#101f30)",borderRadius:14,border:`1px solid ${cfg.color}30`,padding:16,marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {allWeeks.map(w=>{const p=REAL_PRICES[w]?.[origin]?.[cal];const isKg=cal>=26;return(
                <div key={w} style={{flex:1,minWidth:80,background:"#07101a",borderRadius:10,padding:"10px 12px",border:`1px solid ${w===LATEST_WEEK?cfg.color+"40":"#1a2740"}`}}>
                  <div style={{fontSize:10,color:"#4b5563",marginBottom:4}}>S{w}</div>
                  {p?<><div style={{fontFamily:"'Space Mono',monospace",fontSize:13,color:"#4ade80",fontWeight:700}}>{p[0]}–{p[1]}</div><div style={{fontSize:9,color:"#374151"}}>€/{isKg?"kg":"box"}</div></>:<div style={{color:"#374151",fontSize:12}}>{t.nd}</div>}
                </div>);})}
          ...
          <div style={{background:"linear-gradient(135deg,#04200f,#0b1624)",borderRadius:14,border:"1px solid #14532d",padding:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:aiText?14:0,flexWrap:"wrap",gap:10}}>
              ...
              <button onClick={getAI} disabled={aiLoading} style={{background:aiLoading?"#1a2740":"linear-gradient(135deg,#166534,#15803d)",border:"none",borderRadius:9,padding:"10px 20px",color:aiLoading?"#4b5563":"white",fontSize:13,fontWeight:600,cursor:aiLoading?"not-allowed":"pointer",fontFamily:"inherit"}}>
                {aiLoading?t.aiLoading:t.aiBtn}
              </button>
            </div>
            {aiText?<div style={{background:"#07101a",borderRadius:10,padding:14,fontSize:13,lineHeight:1.8,color:"#d1fae5",borderLeft:"3px solid #4ade80"}}>{aiText}</div>
...
export default function App(){
  ...
  if(!isLoaded){return(
    <div style={{minHeight:"100vh",background:"#07101a",display:"flex",alignItems:"center",justifyContent:"center"}}>
